...
<?php
		...
		$MySQL->Select("KDFAKMSFAK","msfakupy","","KDFAKMSFAK ASC","");
		$i=0;
		$jml=$MySQL->num_rows();
		while ($show=$MySQL->fetch_array()) {
		...
		echo "<table border='0' align='center' style='width:99%; cellpadding='2' cellspacing='1' margin:10px auto; overflow:scroll' class='tblbrdr' >";
			/***** Default Tampilan **************/
		echo "<tr><th colspan='7' style='background-color:#EEE'>DAFTAR PROGRAM STUDI</th></tr>";
		echo "<tr>
				<th style='width:20px;'>KODE</th>
				<th>PROGRAM STUDI</th>
				<th style='width:5px;'>JENJANG</th>
				<th style='width:80px;'>TELP.</th>
				<th style='width:200px;'>KA. PROGRAM STUDI</th>
				<th style='width:50px;'>STATUS</th>
				<th style='width:20px;'>ACT</th></tr>";
	
		for ($i=0;$i < $jml;$i++) {
			echo "<tr><td colspan='7' class='fwb' style='background-color:#EEE'>FAKULTAS : ".LoadFakultas_X("",$master[$i])."</td></tr>";
			$MySQL->Select("mspstupy.IDX,mspstupy.IDPSTMSPST,mspstupy.NMPSTMSPST,mspstupy.NMJENMSPST,mspstupy.TELPOMSPST,mspstupy.KAPSTMSPST,mspstupy.STATUMSPST,tbkodupy.NMKODTBKOD","mspstupy","LEFT OUTER JOIN tbkodupy ON (mspstupy.STATUMSPST = tbkodupy.KDKODTBKOD) WHERE mspstupy.KDFAKMSPST ='".$master[$i]."' AND tbkodupy.KDAPLTBKOD ='14'","IDPSTMSPST ASC","");
			if ($MySQL->Num_Rows() > 0){
				$no=1;
				while ($show=$MySQL->Fetch_Array()){
					$sel="";
					...
					echo "<tr>";
			     	...
			     	echo "<td class='$sel tac'>".$show['TELPOMSPST']."</td>";
