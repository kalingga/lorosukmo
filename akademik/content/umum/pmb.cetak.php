...
class PDF extends FPDF{
	...
	function Table($title,$w,$align,$header,$data,$style='1',$show_header='yes',$sub_title=""){
		...
		if (!empty($sub_title)){
			...
			foreach($sub_title_arr as $subtitle){
				$this->Cell(190,7,$subtitle,0,0,'L');
				$this->Ln();	
			}
		}
		$this->SetLineWidth(.3);
	...
	function Sign($space=1){
		...
		$this->Cell(80,5,$_SESSION[$PREFIX.'kabupaten_instansi'].", ".FormatDateTime(time(),2),0,0,'C');
		$this->Ln();
		$this->Cell(110,5,"",0,0,'R');
		$this->Cell(80,5,$_SESSION[$PREFIX.'nama_instansi'],0,0,'C');
		$this->Ln();
		$this->Cell(110,5,"",0,0,'R');
	...
	function Header(){
		global $PREFIX;
		if (file_exists($_SESSION[$PREFIX.'logo_instansi']))
			$this->Image($_SESSION[$PREFIX.'logo_instansi'],9,8,18,20);
		$this->SetTextColor(0);
		$this->SetFont('Arial','B',12);
		$this->Text(30,13,$_SESSION[$PREFIX.'lembaga_instansi']);
		$this->Ln();
		$this->SetFont('Arial','B',18);
		$this->Text(30,20,$_SESSION[$PREFIX.'nama_instansi']);
		$this->Ln();
		$this->SetFont('Arial','B',9);
		$this->Text(30,25,$_SESSION[$PREFIX.'alamat_instansi']);
		$this->Ln(20);
		$this->Cell(190,.5,'','TB');
		$this->Ln(3);
	}
