...
if ($sm_akses_arr[$idpage] == 0) {
	...
	$qry .="AND tbkmkupy.NODOSTBKMK ='".$user_admin."'";	     
	$MySQL->Select("DISTINCT tbmksajiupy.KDKMKMKSAJI,tblmkupy.NAMKTBLMK,tbkmkupy.SKSMKTBKMK,tbkmkupy.SEMESTBKMK,tbkmkupy.NODOSTBKMK,tbkmkupy.NMDOSTBKMK,tbkmkupy.KDPSTTBKMK","tbmksajiupy",$qry,"tbkmkupy.SEMESTBKMK,tbmksajiupy.KDKMKMKSAJI","");
	$jml_data=$MySQL->num_rows();
	if ($jml_data > 0) {
		$i=0;
		while ($show=$MySQL->Fetch_Array()){
	     	$kd_mk[$i] = $show['KDKMKMKSAJI'];
	     	$nm_mk[$i] = $show['NAMKTBLMK'];
	    	$sk_mk[$i] = $show['SKSMKTBKMK'];
	    	$sm_mk[$i] = $show['SEMESTBKMK'];
	    	$ps_mk[$i] = $show['KDPSTTBKMK'];
	        $i++;
	    }
	}
	echo "<tr>
		...
		if ($jml_data > 0){
			...
			for ($i=0; $i < $jml_data;$i++){
				...
				if ($no % 2 == 1) $sel="sel"; 
		     	...
		     	echo "<td class='$sel'>".$kd_mk[$i]."</td>";
		     	echo "<td class='$sel'>".$nm_mk[$i]."</td>";
		    	echo "<td class='$sel tac'>".$sk_mk[$i]."</td>";
		    	echo "<td class='$sel tac'>".$sm_mk[$i]."</td>";
		    	echo "<td class='$sel'>".LoadProdi_X("",$ps_mk[$i])."</td>";
		     	echo "</tr>";
		        $no++;
		    }
		} else {
