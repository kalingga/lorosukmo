...
<?php	
    ...
    echo "<tr><th colspan='6' style='background-color:#EEE'>JADWAL PERKULIAHAN TA. ".substr($ThnSemester,0,4)."/".((substr($ThnSemester,0,4))+1)." SEM. ".LoadSemester_X("",substr($ThnSemester,4,1))."</th></tr>";
		for ($i=0;$i < $jml_master;$i++) {
			echo "<tr><td colspan='6' style='padding-top:5px; padding-bottom:5px;'><a href=\"jadwal/sch/form/".md5('blanc'.$id_pst[$i])."\" class='fancy blanc' style='color:#333; font-weight:bold; padding:3px; border: solid #999 1px;'>Tambah Kelas Baru</a>
					    ...
					    echo "</tr>";	
  						while ($detail=$MySQL->fetch_array()){
    					    ...
    					    echo "<td class='tac $sel1'>";
    							echo "<a class=fancy href='jadwal/sch/edit/0/".$detail['IDX']."'><img border='0' src='images/b_edit.png' title='EDIT DATA' /></a> ";
    				 			echo "</td>";
...
function cekAmbil($id){
   global $MySQL;
   $MySQL->Select("DISTINCT IDX","trnlmupy",
   "WHERE IDXMKSAJI = '$id'","","1","0");
   //echo $MySQL->qry;
   if($MySQL->Num_Rows() > 0){
      return true;
   }else{
