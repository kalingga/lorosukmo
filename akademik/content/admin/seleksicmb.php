...
if ($sm_akses_arr[$idpage] == 0) {
	...
	if (isset($_POST['submit'])){
		...
		$edtDaftar=strip_tags($_POST['edtDaftar']);
		$cbStatus=substr(strip_tags($_POST['cbStatus']),0,1);
		$rbDiterima=substr(strip_tags($_POST['rbDiterima']),0,2);
		if (($cbStatus=='-1') || ($cbStatus=='0')) $rbDiterima="";

		$MySQL->Update("tbpmbupy","DTRMATBPMB='$rbDiterima',STTRMTBPMB='$cbStatus'","where IDX=".$edtID,"1");
			$msg .="Proses Seleksi PMB atas Mahasiswa dengan No. Pendaftaran '".$edtDaftar."' Berhasil!";
			// perintah SQL berhasil dijalankan
  		if ($MySQL->exe){
			$succ=1;
		}
  		if ($succ==1){
	  		$act_log="Update ID='$id_admin' Pada Tabel 'tbpmbupy' File 'seleksicmb.php'  Oleh User=".$_SESSION[$PREFIX.'user_admin']." Sukses!";
    		AddLog($_SESSION[$PREFIX.'id_admin'],$act_log);
	    	echo "<div id='msg_err' class='divsucc m5 p5 tac'>";
	    	echo $msg;
	    	echo "</div>";
		} else {
	  		$act_log="Update ID='$id_admin' Pada Tabel 'tbpmbupy' File 'seleksicmb.php'  Oleh User=".$_SESSION[$PREFIX.'user_admin']." Gagal!";
			AddLog($_SESSION[$PREFIX.'id_admin'],$act_log);
    		echo "<div id='msg_err' class='diverr m5 p5 tac'>";
	    	echo "Maaf!, Proses Seleksi PMB atas Mahasiswa dengan No. Pendaftaran '".$edtDaftar."' Tidak Berhasil!";
	    	echo "</div>";
		}
	 }
...
?>
	...
	<table style="width:99%" border="0" cellpadding="1" cellspacing="1">
		...
		<tr><td colspan="2">Telah Mengikuti Semua Tahapan Seleksi PMB di Universitas PGRI Yogyakarta dan dinyatakan : 
		<?php 
		echo "<select name='cbStatus' id='' required='1' exclude='-1' err='Pilih Salah Satu Dari Daftar Status Penerimaan CMB bersangkutan'>";
		echo "<option value='-1' selected='selected' >--- Status ---</option><option value=0 >Tidak Diterima</option>";
		
		if($_SESSION[$PREFIX.'id_group'] <> "8"){
			echo "<option value=1 >Diterima Melalui Test</option>";
			echo "<option value=2 >Diterima Bebas Test</option>";
		}
		echo "<option value=3 >Lulus Tes Tertulis</option></select>";
		//LoadKode_X("cbStatus","$cbStatus","83","required='1' exclude='-1' err='Pilih Salah Satu Dari Daftar Status Penerimaan CMB bersangkutan'") 
		?>
		</td></tr>
		<tr>
...
<button type='button' onClick=window.location.href='./?page=seleksicmb'><img src='images/b_cancel.gif' class='btn_img'/>&nbsp;Batal</button>&nbsp;		    	
<button type='submit' name="submit" value="submit"><img src='images/b_save.gif' class='btn_img'/>&nbsp;PROSES</button>&nbsp;		    	
			</td>
		</tr>
	</table>
	</form>
		...
		$MySQL->Select("tbpmbupy.*,mspstupy.NMPSTMSPST AS PILIHAN1,mspstupy1.NMPSTMSPST AS PILIHAN2,mspstupy2.NMPSTMSPST AS DITERIMA","tbpmbupy",$qry,"IDX ASC","","0");
	 	...
	 	if ($jumlah_row > 0){	
			...
			//$PILIHAN2= LoadProdi_X("",$show['PLHN2TBPMB']);
				...
				echo "<a href='./?page=seleksicmb&amp;p=edit&amp;id=".$row[$i][0]."'><img border='0' src='images/b_edit.png' title='EDIT DATA' /></a> ";
	     		echo "</td>";
	     		echo "</tr>";
	     		$no++;
	     	}
