...
class DHTML_Calendar {
    ...
    function DHTML_Calendar($calendar_lib_path = '/calendar/',
                            ...
                            $stripped          = true) {
        if ($stripped) {
            $this->calendar_file = 'calendar_stripped.js.php';
            $this->calendar_setup_file = 'calendar-setup_stripped.js.php';
        } else {
            $this->calendar_file = 'calendar.js.php';
            $this->calendar_setup_file = 'calendar-setup.js.php';
        }
        $this->calendar_lang_file = 'lang/calendar-' . $lang . '.js.php';
    ...
    function make_input_field($cal_options = array(), $field_attributes = array()) {
        ...
        $attrstr = $this->_make_html_attr(array_merge($field_attributes,
                                                      array('id'   => $this->_field_id($id),
                                                            'type' => 'text')));
        echo '<input ' . $attrstr .' maxlength="10" size="10" />';
        echo '<a href="#" id="'. $this->_trigger_id($id) . '">' .
            '<img alt="CAL" title="Klik Untuk Memunculkan Kalender" border="0" src="' . $this->calendar_lib_path . 'cal.gif" style="margin:0 2px;" height="16" width="18" /></a> dd-mm-yyyy';
