...
class PDF extends FPDF{
	...
	function Page($w,$data,$photo){
		...
		$this->Cell($w[0],5,"NO.","1","","","1");
		$this->Cell($w[1],5,"NIM","1","","","1");
		$this->Cell($w[2],5,"MAHASISWA","1","","","1");
		$this->Cell($w[3],5,"PROGRAM STUDI","1","","","1");
		$this->Cell($w[4],5,"TAHUN MASUK","1","","C","1");
		$this->Ln();
		$i= 1;
		foreach($data as $row){
			if ($i % 56 == 0) {
/*				$this->Cell($w[0],5,"","0","1","","1");
				$this->Cell($w[1],5,"","0","1","","1");
				$this->Cell($w[2],5,"","0","1","","1");
				$this->Cell($w[3],5,"","0","1","","1");
				$this->Cell($w[4],5,"","0","1","C","1");*/
				$this->AddPage();
				$this->SetFillColor(223,223,223);
				$this->Cell($w[0],5,"NO.","1","","","1");
				$this->Cell($w[1],5,"NIM","1","","","1");
			...
			$this->Cell($w[0],5,$i,"LR");
			$this->Cell($w[1],5,$row[0],"LR");
			$this->Cell($w[2],5,$row[1],"LR");
			$this->Cell($w[3],5,$row[2],"LR");
			$this->Cell($w[4],5,$row[3],"LR");
			$this->Ln();
			$i++;
		}
	}
...
if (isset($_POST['report_title'])){
	...
	switch ($report_title) {
		...
		case '1' :
			...
			$width1="12,18,50,7,8,20,20,20,20,20";
			$width1= @explode(",",$width1);
			$pdf->Page($width1,$data);
			break;
	}
