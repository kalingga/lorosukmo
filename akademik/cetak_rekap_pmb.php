...
class PDF extends FPDF{
	...
	function Page($w,$data,$photo){
		...
		$this->Cell($w[2],5,"PILIHAN 1 / PILIHAN 2","1","","","1");
		$this->Cell($w[4],5,"TTD","1","","C","1");
		$this->Ln();
		...
		foreach($data as $row){
			...
			$this->Cell($w[2],5,$row[2]." /","LR");
			$this->Cell($w[4],5,$row[4],"LR");
			$this->Ln();
			$this->Cell($w[0],5,"","LRB");
			$this->Cell($w[1],5,"","LRB");
			$this->Cell($w[2],5,$row[3],"LRB");
			$this->Cell($w[4],5,"","LRB");
			$this->Ln();
		}
...
if (isset($_POST['tgl']) && $_POST['tgl'] != ""){
	...
	$pdf->Cell(190,2,"per Tanggal ".DateStr($_POST['tgl']),0,0,'R');
}
