...
if ($sm_akses_arr[$idpage] == 0) {
	...
	if ($id_group=='2') {
		$edtNIM=$user_admin;
		if(getBPMStatus($edtNIM, $krsBefore) ){ // AND (substr($edtNIM,0,2) <> '12')
	...
	if (isset($_GET['p']) && ($_GET['p']=='delete')) {
    	...
    	if ($MySQL->exe){
			$act_log="Hapus ID='$id' Pada Tabel 'trnlmupy' File 'isiankrs.php' Sukses!";
			AddLog($id_admin,$act_log);
	    	echo $msg_delete_data;
		} else {
			$act_log="Hapus ID='$id' Pada Tabel 'trnlmupy' File 'isiankrs.php.php' Gagal!";
		echo '<br/>'.$MySQL->show_error();
			AddLog($id_admin,$act_log);
	...
	pengisian KRS *****/
?>
		<form name="form1" action="./?page=isiankrs" method="post" enctype="multipart/form-data" onsubmit="return ValidateForm(this)">
		<table style="width:95%" border="0" cellpadding="1" cellspacing="1">
			<tr><th colspan="2">Form Isian KRS</th></tr>
...
<?php
	} else {  // jika ada query string submit, p, atau nim dan login sbg mahasiswa
		if ($_POST['submit']=='simpan') {
			$MySQL->insert("tbkrsupy","THSMSTBKRS,NIMHSTBKRS,KDPSTTBKRS,DIACCTBKRS","'$LastKRS','$edtNIM','$prodi','0'");
			$act_log="Tambah Data Pada Tabel 'tbkrsupy' File 'isiankrs.php' ";
			if ($MySQL->exe) {
      ...
      $batasSKS = CekLimitSKS($edtNIM,$periodeSblm,($jml_sks_now + $sks));		
			//echo "TAHUN KRS : ".$thn; // last edit, datanya yang belum terupdate ke semester sekarang, shg berhasil ditambahkan, tapi tidak nampil
			// === KAPASITAS KELAS ============
			
      $batasKapasitas = CekKapasitas($LastKRS, $id);
			
			if ($prasyarat AND $batasSKS AND $batasKapasitas) {
				$succ=0;
				...
				$act_log="Tambah Data Pada Tabel 'trnlmupy' File 'isiankrs.php' ";
				if ($MySQL->exe){
					$succ=1;
				}
				if ($succ==1) {
					...
					//echo "trnlmupy - THSMSTRNLM,NIMHSTRNLM,IDKMKTRNLM,KDKMKTRNLM,IDXMKSAJI,KELASTRNLM,SKSMKTRNLM,SEMESTRNLM,STAMKTRNLM"."'$thn','$edtNIM','$idkmk','$mk','$idx','$kelas','$sks','$semes','$statusMK'";
					$msg=$msg_update_0;
					$act_log .="Gagal!";
				}
				echo $msg;
		...
		****** Program Studi dan Tahun Semester Mahasiswa Bersangkutan ****/
			...
			echo "<option value='tbkmkupy.SEMESMKSAJI' $sel2 >SEMESTER</option>";
			echo "</select>";
			...
			$qry .=" WHERE tbmksajiupy.THSMSMKSAJI = '".$LastKRS."' AND tbmksajiupy.KDPSTMKSAJI='".$prodi."' AND SEMESMKSAJI <> ''";
			...
			//	    	while ($show=$MySQL->Fetch_Array()){
				...
				ruangNama AS RUANGMKSAJI,tbmksajiupy.HRPELMKSAJI,tbmksajiupy.MULAIMKSAJI,tbmksajiupy.DURSIMKSAJI,tbmksajiupy.SMPAIMKSAJI",
				"tbmksajiupy",
				"LEFT JOIN ruang_kuliah ON ruangId = tbmksajiupy.RUANGMKSAJI 
        WHERE tbmksajiupy.KDKMKMKSAJI = '".$data[$i][1]."' 
				AND tbmksajiupy.THSMSMKSAJI = '".$LastKRS."' AND HRPELMKSAJI IS NOT NULL",
...
//			echo $MySQL->qry;
				    ...
				    echo "<tr>";
			   		echo "<th style='width:40px;'>KLS</th> 
						...
						<th style='width:180px;'>DOSEN PENGAMPU</th>
					...
					while ($detail=$MySQL->fetch_array()){
						...
						echo "<a href='./?page=isiankrs&amp;p=ins&amp;nim=".$_GET['nim']."&amp;id=".$detail['IDX']."'><img border='0' src='images/b_go.png' title='INSERT DATA' /></a> ";
			 			echo "</td></tr>";
						$no1++;	
					}
			     	echo "</table>";
			...
			echo "</td></tr>";
			echo "</table>";
			echo "<center><button type='button' onClick=window.location.href='./?page=isiankrs&amp;nim=$edtNIM' /><img src='images/b_back.png' class='btn_img'>&nbsp;Kembali</button></center><br>";
		} else {
			$akses=1;
			if ($id_group=="2") {
...
?>
			...
			<tr><td colspan='9' style='background-color:#FFF'><button type='button' onClick=window.location.href='./?page=isiankrs&amp;p=add&amp;nim=<?php echo $nim; ?>' /><img src='images/b_add.png' class='btn_img'>&nbsp;Tambah Data</button></td></tr>
				<tr>
					<th style="width:20px">No</th>
...
<?php	
				$MySQL->Select("trnlmupy.IDX,trnlmupy.KDKMKTRNLM,tblmkupy.NAMKTBLMK,trnlmupy.KELASTRNLM,trnlmupy.SEMESTRNLM,trnlmupy.SKSMKTRNLM,trnlmupy.STAMKTRNLM,tbmksajiupy.NODOSMKSAJI,tbmksajiupy.NMDOSMKSAJI","trnlmupy","LEFT OUTER JOIN tblmkupy ON (trnlmupy.KDKMKTRNLM = tblmkupy.KDMKTBLMK) LEFT OUTER JOIN tbmksajiupy ON (trnlmupy.IDXMKSAJI = tbmksajiupy.IDX) WHERE trnlmupy.THSMSTRNLM = '".$LastKRS."' AND  trnlmupy.NIMHSTRNLM = '".$edtNIM."' AND trnlmupy.ISKONTRNLM <> '1'","trnlmupy.KDKMKTRNLM ASC");
				$jmlSKS=0;
				$i=0;
				while ($show=$MySQL->fetch_array()){
					$data[$i][0]=$show["IDX"];
        ...
        $_SESSION['jml_sks'] = $jmlSKS;  			
				...
				for ($j=0;$j < 12;$j++ ){
					...
					echo "<tr><td class='$sel1 tac'>".$no."</td>
						...
						if ($flag[$j]=="1") {
							if ($data[$j][7] != "1") {
							echo "<a href='./?page=isiankrs&amp;p=delete&amp;nim=".$nim."&amp;id=".$data[$j][0]."' ><img border='0' src='images/b_drop.png' title='HAPUS DATA' onclick=\"return confirm('Apakah Anda Yakin Akan Menghapus Data Ini?')\"/></a>";
							} else {
			...
			//************* Cetak KRS ********************//
				...
				echo "<form action='./?page=isiankrs' method='post'>";
				echo "<input type='hidden' name='edtNIM' value='$edtNIM' />
					<button type='reset' name='reset' onClick=window.location.href='./?page=isiankrs'><img src='images/b_back.png' class='btn_img'/>&nbsp;Kembali</button>				
...
function CekWaktuKRS($curr_akses,$thn){
	global $MySQL;
	$MySQL->Select("MLKRSSETKRS,BTKRSSETKRS,TGAWLSETKRS,TGAKHSETKRS ","setkrsupy","where TASMSSETKRS='".$thn."'","","1");
//echo $MySQL->qry;
	$show=$MySQL->fetch_array();
	$mulaikrs=$show["MLKRSSETKRS"];
	$bataskrs=$show["BTKRSSETKRS"];
	$mulaiPerubahan = $show["TGAWLSETKRS"];
	$akhirPerubahan = $show["TGAKHSETKRS"];
	$curr_akses= New Date($curr_akses);
	$curr_akses_1=$curr_akses->isBetween($mulaikrs,$bataskrs);
	$curr_akses_2=$curr_akses->isBetween($mulaiPerubahan,$akhirPerubahan);

	if ((!$curr_akses_1) AND (!$curr_akses_2)) {
		echo "<div id='msg_err' class='diverr m5 p5 tac'>";
		echo "Maaf!, Waktu Pengisian KRS Telah Melewati Batas Akhir!";
...
function cekKapasitas($LastKRS, $id){
  global $MySQL;
	$MySQL->Select("COUNT(*) AS KAP",
		"trnlmupy","WHERE IDXMKSAJI = '".$id."' AND THSMSTRNLM = '".$LastKRS."'","","");
	//echo $MySQL->qry;
  $show=$MySQL->fetch_array();
	$jmlpeserta=$show["KAP"];
	// ==========================
	$MySQL->Select("KAPASITAS",
		"tbmksajiupy","WHERE IDX = '".$id."'","","1");
	//echo $MySQL->qry;

  $res=$MySQL->fetch_array();
	$kapasitas = $res["KAPASITAS"];
	
	
	if ($jmlpeserta < $kapasitas) {
return true;
	} else {
	  echo "<div id='msg_err' class='diverr m5 p5 tac'>";
    echo "<b>Kapasitas kelas telah penuh, silakan pilih kelas yang lain</b>";
    echo "</div>";
	  return false;
  }
